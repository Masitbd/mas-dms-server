...
const getAllStocks = async (): Promise<IStock[]> => {
  const result = await Stock.find({}).populate('productId').populate('purchaseItemId');
  return result;
};

const getSingleStock = async (id: string): Promise<IStock | null> => {
  const result = await Stock.findById(id).populate('productId').populate('purchaseItemId');
  return result;
};
