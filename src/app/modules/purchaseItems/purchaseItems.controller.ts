...
const getSinglePurchaseItem = catchAsync(
...
const getPurchaseItemForSinglePurchase = catchAsync(
  async (req: Request, res: Response) => {
    const { id } = req.params;
    const result = await PurchaseItemService.getPurchaseItemForSinglePurchase(
      id
    );
